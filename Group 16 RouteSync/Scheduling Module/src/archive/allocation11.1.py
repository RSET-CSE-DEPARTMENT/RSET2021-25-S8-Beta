...
def fitness(allocation, trips, stops):
    ...
    logging.info(
        f"Fitness: {fitness_value:.6f}, Bus Utilization: {bus_utilization} | {bus_unutilization}, Overlaps: {overlap_penalty:.6f}, "
...
def genetic_algorithm(trips, stops, max_buses, generations=100, export_interval=5):
    ...
    for gen in range(generations):
        ...
        # Prepare data for export
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = f"collected_schedules3/best_schedule_bus{max_buses}_gen{gen + 1}.csv"
        allocation_data = []
        for i, bus_schedule in enumerate(best_allocation):
            for trip_id in bus_schedule:
...
if __name__ == "__main__":
    ...
    max_buses_list = [400, 600, 800, 1000]  # Example: different max buses
    ...
    for max_buses in max_buses_list:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = f"collected_schedules3/final_bus_schedule_{max_buses}buses_{timestamp}.csv"
