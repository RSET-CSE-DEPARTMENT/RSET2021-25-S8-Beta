...
# Fitness function using precomputed distances
# def fitness(allocation, trips, stop_times, stops, distance_dict):
#     idle_time = 0
#     total_distance = 0

#     print("Starting fitness computation...")
#     for bus_idx, bus_schedule in enumerate(allocation):
#         print(f"\nProcessing bus schedule {bus_idx + 1}/{len(allocation)}")
        
#         # Sort the bus schedule by the earliest arrival time of its trips
#         bus_schedule.sort(key=lambda trip: stop_times[stop_times['trip_id'] == trip]['arrival_time'].min())
#         print(f"Sorted bus schedule: {bus_schedule}")
...
#             print(f"\nTrip {i + 1}: {trip1}, Last Stop: {last_stop_trip1['stop_id']}")
#             print(f"Trip {i + 2}: {trip2}, First Stop: {first_stop_trip2['stop_id']}")

#             # Fetch precomputed distance from the dictionary
#             if (last_stop_trip1['stop_id'], first_stop_trip2['stop_id']) in distance_dict:
#                 travel_time_between_trips = distance_dict[(last_stop_trip1['stop_id'], first_stop_trip2['stop_id'])]
#             else:
#                 # If the reverse pair exists in the dictionary
#                 travel_time_between_trips = distance_dict[(first_stop_trip2['stop_id'], last_stop_trip1['stop_id'])]

#             print(f"Travel time between trips (minutes): {travel_time_between_trips}")
...
#             print(f"Time difference between trips (minutes): {time_diff}")

#             # Calculate idle time increment
#             idle_time_increment = max(0, time_diff - travel_time_between_trips)
#             idle_time += idle_time_increment
#             print(f"Idle time increment: {idle_time_increment}, Total idle time so far: {idle_time}")

#             # Use the precomputed distance for the total distance
#             distance_increment = travel_time_between_trips  # Assuming this is already in km
#             total_distance += distance_increment
#             print(f"Distance increment: {distance_increment:.2f} km, Total distance so far: {total_distance:.2f} km")

#     # Calculate fitness value (minimize idle time and distance)
#     fitness_value = 1 / (1 + idle_time + total_distance)
#     print(f"\nFinal fitness value: {fitness_value:.6f}")
...
def fitness(allocation, trips, stop_times, stops):
    ...
    for bus_idx, bus_schedule in enumerate(allocation):
        ...
        for i in range(len(bus_schedule) - 1):
            ...
            #     print(f"Overlap detected between trips {trip1} and {trip2}: {overlap_duration} minutes penalty added.")

    # Modify the fitness value to include the overlap penalty
    # fitness_value = 1 / (1 + idle_time + total_distance + overlap_penalty)
    fitness_value = 1 / (idle_time + total_distance)
    print(f"Final fitness value: {fitness_value:.6f}, Idle Time: {idle_time}, Distance: {total_distance}")
