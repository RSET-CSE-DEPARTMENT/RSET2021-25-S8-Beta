...
def fitness(allocation, trips, stops):
    ...
    # Dynamically compute normalization factors
    max_idle_time = sum((pd.to_datetime(trip_data['end_time_y']) - pd.to_datetime(trip_data['start_time_y'])).dt.total_seconds() / 60)
    max_total_distance = 500000  # Rough estimate, can be refined
    max_overlap_penalty = len(trip_data) * 0.1
    max_infeasibility_penalty = len(trip_data) * 0.5
    ...
    with tqdm(total=total_steps, desc="Calculating Fitness", unit="pair") as pbar:
        for bus_schedule in allocation:
            for i in range(len(bus_schedule) - 1):
                ...
                actual_idle_time += max(0, time_diff)
    ...
    # Fitness value: balance all factors
    fitness_value = (
        (active_time / total_time) - 
        (0.5 * normalized_idle_time) - 
        (0.3 * normalized_total_distance) - 
        (0.1 * normalized_overlap_penalty) - 
        (0.1 * normalized_infeasibility_penalty)
    )
