...
# Crossover
def crossover(trips, parent1, parent2):
    trip_dict = {
        trip_id: {
            "start_time_y": row["start_time_y"],
            "end_time_y": row["end_time_y"]
    ...
    child1, child2 = [], []
    for bus1, bus2 in zip(parent1, parent2):
        combined = bus1 + bus2
        child1_schedule = []
        child2_schedule = []
        for trip in combined:
            ...
            if all(
                trip_dict[trip]["start_time_y"] >= trip_dict[assigned]["end_time_y"] + 10 * 60 or
                trip_dict[trip]["end_time_y"] <= trip_dict[assigned]["start_time_y"] - 10 * 60
                for assigned in child1_schedule
            ):
                child1_schedule.append(trip)
            elif all(
                trip_dict[trip]["start_time_y"] >= trip_dict[assigned]["end_time_y"] + 10 * 60 or
                trip_dict[trip]["end_time_y"] <= trip_dict[assigned]["start_time_y"] - 10 * 60
                for assigned in child2_schedule
            ):
                child2_schedule.append(trip)
        child1.append(child1_schedule)
        child2.append(child2_schedule)
    return child1, child2

def can_add_trip(schedule, trip, trip_dict):
    trip_start = trip_dict[trip]["start_time_y"]
    trip_end = trip_dict[trip]["end_time_y"]
    for assigned in schedule:
        assigned_start = trip_dict[assigned]["start_time_y"]
        assigned_end = trip_dict[assigned]["end_time_y"]
        if not (trip_start >= assigned_end + 10 * 60 or trip_end <= assigned_start - 10 * 60):
            return False
    return True
...
def mutate(allocation, trip_ids, trip_dict):
    for _ in range(random.randint(1, 5)):
        ...
        if allocation[bus1]:
            ...
            if can_add_trip(allocation[bus2], trip, trip_dict):
                allocation[bus2].append(trip)
            else:
                allocation[bus1].append(trip)  # Restore if mutation fails
    return allocation
...
def genetic_algorithm(trips, stops, max_buses, generations=100, export_interval=5):
    ...
    for gen in range(generations):
        ...
        for i in range(0, len(population) - 1, 2):
            parent1, parent2 = population[i], population[i + 1]
            child1, child2 = crossover(trips, parent1, parent2)
            new_population.extend([child1, child2])
...
if __name__ == "__main__":
    ...
    trips['start_time_y'] = pd.to_datetime(trips['start_time_y'], format=datetime_format).view('int64')
    trips['end_time_y'] = pd.to_datetime(trips['end_time_y'], format=datetime_format).view('int64')
