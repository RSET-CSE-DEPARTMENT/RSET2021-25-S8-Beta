...
def initialize_population(trips, max_buses):
    ...
    for i in range(25):  # Population size
        trip_ids = list(trip_dict.keys())
        random.shuffle(trip_ids)  # Randomize trip order
        
        allocation = [[] for _ in range(max_buses)]
        bus_heap = []
        
        for bus in range(max_buses):
            initial_availability = random.randint(0, 10 * 60)  # Randomize bus availability
            heapq.heappush(bus_heap, (initial_availability, bus))
        ...
        # Introduce random mutations to enhance diversity
        for _ in range(random.randint(1, 5)):  # Randomly mutate 1 to 5 trips
            from_bus = random.choice(range(max_buses))
            to_bus = random.choice(range(max_buses))
            if allocation[from_bus]:
                trip = allocation[from_bus].pop()
                allocation[to_bus].append(trip)

        logging.info(f"Population {i}: {unallocated} trips not allocated...")
...
if __name__ == "__main__":
    ...
    max_buses_list = [800, 00, 1000]  # Example: different max buses
