    ...
    for i in range(25):  # Population size
        ...
        for trip in trip_ids:
            ...
            for _, bus in bus_heap:
                if allocation[bus] and trip_start < trip_dict[allocation[bus][-1]]["end_time_y"] + 50 * 60:
...
def fitness(allocation, trips, stops):
    ...
    fitness_value = (
        bus_utilization_normalized + activity_ratio 
    ) / max(overlap_penalty_normalized, 1)
...
def genetic_algorithm(trips, stops, max_buses, generations=100, export_interval=1):
    logging.info("Initializing genetic algorithm...")
    population = initialize_population(trips, max_buses)
    fitness_cache = {}
    
    def get_fitness(ind):
        key = tuple(tuple(bus) for bus in ind)  # Use tuple representation for caching
        if key not in fitness_cache:
            fitness_cache[key] = fitness(ind, trips, stops)
    ...
    for gen in range(generations):
        ...
        if (gen + 1) % export_interval == 0:
            ...
            best_allocation = population[fitnesses.index(best_fitness)]
            
            # Prepare data for export
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_file = f"schedule6/best_schedule_bus{max_buses}_gen{gen + 1}_{timestamp}.csv"
            allocation_data = []
...
if __name__ == "__main__":
    ...
    for max_buses in max_buses_list:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = f"schedule6/bus_schedule_{max_buses}buses_{timestamp}.csv"
