    ...
    for i in range(25):  # Population size
        ...
        for trip in trip_ids:
            ...
            for _, bus in bus_heap:
                if allocation[bus] and trip_start < trip_dict[allocation[bus][-1]]["end_time_y"]:
...
def fitness(allocation, trips, stops):
    """
    Calculate the fitness of a given bus allocation.

    Fitness is based on minimizing idle time, travel distance, and overlapping trips.
    """
    idle_time = 0
    total_distance = 0
    overlap_penalty = 0
    ...
    max_idle_time = 200000  # Set an upper bound for idle time based on expected range
    max_total_distance = 200000  # Set an upper bound for total distance (you may need to calculate this based on your data)
    max_overlap_penalty = 1000  # Set an upper bound for overlap penalties (you can calculate the maximum penalty)
        ...
        for bus_schedule in allocation:
            for i in range(len(bus_schedule) - 1):
                ...
                if trip1_end_time > trip2_start_time:
                    print(pd.to_datetime(trip1['end_time_y']), pd.to_datetime(trip2['start_time_y']))
                    overlap_penalty += 1
    ...
    normalized_idle_time = min(idle_time / max_idle_time, 1)  # Scale the idle time between 0 and 1
    normalized_total_distance = min(total_distance / max_total_distance, 1)  # Scale the total distance between 0 and 1
    normalized_overlap_penalty = min(overlap_penalty / max_overlap_penalty, 1)  # Scale overlap penalty between 0 and 1

    # Fitness value: lower idle time, distance, and overlap are better
    fitness_value = 1 / (1 + normalized_idle_time + normalized_total_distance + 6 * normalized_overlap_penalty)

    logging.info(
        f"Fitness: {fitness_value:.6f}, Idle Time: {idle_time / max_idle_time} {normalized_idle_time}, Distance: {total_distance / max_total_distance} {normalized_total_distance}, Overlaps: {normalized_overlap_penalty}"
    )
